...
import { Lock, ArrowRight, Loader2, Car, ChevronLeft, CheckCircle2, Eye, EyeOff, Phone } from 'lucide-react';
...
export default function PortalLoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      toast.success('Access granted. Redirecting to member dashboard...');
      navigate('/portal/dashboard');
    } catch (err) {
      toast.error(err.response?.data?.error || 'Invalid phone number or password.');
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#08080c] relative overflow-hidden">
      ...
      <div className="w-full max-w-[500px] relative z-10 animate-in fade-in slide-in-from-bottom-8 duration-1000 ease-out">
        <div className="glass-morphism rounded-[48px] p-12 md:p-14 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] border border-white/5 bg-[#0c0c12]/95 backdrop-blur-3xl">
            ...
            <div className="space-y-6">
              <div className="group space-y-4">
                ...
                <div className="relative">
                  ...
                  />
                  <Phone className={`absolute right-6 top-1/2 -translate-y-1/2 w-4.5 h-4.5 transition-colors duration-300 ${isPhoneValid ? 'text-customer' : 'text-white/20'}`} />
                </div>
              </div>
                ...
                <div className="flex items-center justify-between px-1">
                  <label className="flex items-center gap-3 cursor-pointer group/check">
                    <div className="relative w-5 h-5">
                      ...
                      />
                      <div className="w-full h-full border border-white/10 rounded-md bg-white/5 transition-all peer-checked:bg-customer peer-checked:border-customer" />
                  ...
                  <Link to="/portal/forgot-password" size="sm" className="text-[11px] font-bold text-customer/60 hover:text-customer transition-colors uppercase tracking-widest underline underline-offset-4">
                    Forgot Password?
                  </Link>
                </div>
              </div>
          ...
          <div className="mt-12 pt-10 border-t border-white/5 text-center flex flex-col items-center gap-6">
            <Link to="/login" className="inline-flex items-center gap-3 text-[11px] font-black text-white/20 uppercase tracking-[0.3em] hover:text-white transition-all duration-300 group">
        ...
        <p className="mt-12 text-center text-[10px] font-black text-white/10 uppercase tracking-[0.5em]">
          Infrastructure Guard &copy; 2026
        </p>
      </div>
    </div>
  );
